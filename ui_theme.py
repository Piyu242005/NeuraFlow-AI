...
:root {{
  --nf-red: {RED};
  --nf-red-dark: {RED_DARK};
  --nf-black: {BLACK};
  --nf-surface: {SURFACE};
  --nf-surface-2: {SURFACE_2};
  --nf-text: {TEXT};
  --nf-muted: {MUTED};
}}
...
section[data-testid="stSidebar"] {{
  ...
  border-right: 1px solid rgba(220,38,38,.22) !important;
}}
...
section[data-testid="stSidebarNav"] a[aria-current="page"] {{
  background: linear-gradient(90deg, rgba(220,38,38,.20), rgba(220,38,38,.07)) !important;
  color: #ffffff !important;
  border-left: 3px solid {RED} !important;
  font-weight: 700 !important;
}}
section[data-testid="stSidebarNav"] a[aria-current="page"] span {{ color: #ffffff !important; }}
...
.nf-page-header {{
  ...
  background: linear-gradient(135deg, rgba(220,38,38,.10), rgba(0,0,0,.20));
...
@media (max-width:768px) {{
  ...
  .nf-page-header {{ padding:14px; }}
  .nf-page-title {{ font-size:21px; }}
}}
</style>
""",
...
def plotly_layout(fig, height=320):
    """Apply the same black/red Plotly theme to an existing figure."""
    fig.update_layout(
        ...
        coloraxis=dict(colorscale=[[0, RED_DARK], [0.5, RED], [1, "#ff6b6b"]]),
