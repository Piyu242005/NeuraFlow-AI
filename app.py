...
# IMPORTANT: load the actual design system used by the redesigned UI.
...
selected = st.radio(
    "Provider", list(PROVIDERS.keys()), index=0, horizontal=True,
    label_visibility="collapsed", key="selected_provider",
)
...
search_provider = st.radio("Search Provider", ["Auto", "Tavily", "DuckDuckGo"], index=0, horizontal=True, key="search_provider")
...
if not uploaded:
    st.markdown('<div class="empty-state"><div class="icon">📄</div><h3>Upload a document to begin</h3><p>Drop a PDF here and NeuraFlow will index it for semantic Q&A.</p></div>', unsafe_allow_html=True)
    st.stop()
...
if m:
    ...
    st.markdown(f'<div class="file-card"><div style="font-size:24px;">📄</div><div><div class="file-name">{uploaded.name}</div><div class="file-meta">{m["pages"]} pages · {cache_badge}</div></div></div>', unsafe_allow_html=True)
    cols = st.columns(4)
    ...
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            ...
            with st.chat_message("assistant"):
                ...
                badge = f"Auto Agent → **{prov}**" if mode == "auto" else f"Powered by **{prov}**"
                st.caption(badge)
                st.write(msg["content"])
...
with st.form("question_form", clear_on_submit=True):
    question = st.text_input("Question", placeholder="Ask anything about the document…", label_visibility="collapsed")
...
if final_q:
    if not providers:
        ...
        if enable_tools:
            ...
            wrapper = executor.run_react_stream(user_question=final_q, memory_context=mem_ctx, status_callback=update_status, show_reasoning=show_reasoning)
